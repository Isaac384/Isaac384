<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Presentación GitHub - Ismael Jimenez</title>
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;600;700&display=swap" rel="stylesheet">
  <script src="https://cdnjs.cloudflare.com/ajax/libs/particles.js/2.0.0/particles.min.js"></script>
  <style>
    * { margin:0; padding:0; box-sizing:border-box; font-family:'Poppins',sans-serif; }
    body {
      background: linear-gradient(135deg,#0f172a,#1e293b,#020617);
      color:#fff;
      overflow-x:hidden;
    }
    #particles-js {
      position:fixed;
      width:100%; height:100%;
      z-index:-1;
    }
    .container {
      min-height:100vh;
      display:flex;
      justify-content:center;
      align-items:center;
      padding:2rem;
    }
    .card {
      max-width:900px;
      background:rgba(255,255,255,0.08);
      backdrop-filter: blur(15px);
      border:1px solid rgba(255,255,255,0.15);
      border-radius:25px;
      padding:3rem;
      text-align:center;
      box-shadow:0 20px 60px rgba(0,0,0,0.5);
      animation: fadeUp 1.2s ease;
    }
    .avatar {
      width:140px;
      height:140px;
      border-radius:50%;
      border:4px solid #38bdf8;
      margin-bottom:1rem;
      box-shadow:0 0 30px rgba(56,189,248,.6);
      animation: float 3s infinite ease-in-out;
    }
    h1 {
      font-size:3rem;
      background: linear-gradient(90deg,#38bdf8,#818cf8,#22d3ee);
      -webkit-background-clip:text;
      -webkit-text-fill-color:transparent;
    }
    .subtitle {
      font-size:1.2rem;
      color:#cbd5e1;
      margin:1rem 0;
    }
    .description {
      font-size:1rem;
      line-height:1.8;
      color:#e2e8f0;
      margin:1.5rem 0;
    }
    .skills {
      display:flex;
      flex-wrap:wrap;
      justify-content:center;
      gap:12px;
      margin:2rem 0;
    }
    .badge {
      padding:.7rem 1.2rem;
      border-radius:999px;
      background:rgba(56,189,248,.15);
      border:1px solid rgba(56,189,248,.35);
      transition:.3s;
    }
    .badge:hover {
      transform:scale(1.08);
      box-shadow:0 0 20px rgba(56,189,248,.4);
    }
    .buttons {
      margin-top:2rem;
      display:flex;
      gap:1rem;
      justify-content:center;
      flex-wrap:wrap;
    }
    .btn {
      padding:1rem 1.8rem;
      border:none;
      border-radius:14px;
      text-decoration:none;
      font-weight:600;
      transition:.3s;
    }
    .primary {
      background:#38bdf8;
      color:#0f172a;
    }
    .secondary {
      background:transparent;
      color:#fff;
      border:1px solid rgba(255,255,255,.3);
    }
    .btn:hover {
      transform:translateY(-4px);
      box-shadow:0 10px 25px rgba(0,0,0,.35);
    }
    @keyframes fadeUp {
      from { opacity:0; transform:translateY(40px); }
      to { opacity:1; transform:translateY(0); }
    }
    @keyframes float {
      0%,100% { transform:translateY(0); }
      50% { transform:translateY(-10px); }
    }
    @media(max-width:768px){
      h1 { font-size:2rem; }
      .card { padding:2rem; }
    }
  </style>
</head>
<body>
  <div id="particles-js"></div>

  <div class="container">
    <div class="card">
      <img class="avatar" src="https://avatars.githubusercontent.com/u/1?v=4" alt="Avatar">
      <h1>Ismael Jimenez</h1>
      <p class="subtitle">💻 Desarrollador de Software | Backend Developer | API & Web Apps</p>
      <p class="description">
        Desarrollador de Software Junior enfocado en backend y aplicaciones web escalables.
        Experiencia en Python, PHP, JavaScript, SQL, integración de APIs RESTful,
        pasarelas de pago y desarrollo full-stack moderno.
      </p>

      <div class="skills">
        <span class="badge">Python</span>
        <span class="badge">PHP</span>
        <span class="badge">JavaScript</span>
        <span class="badge">React</span>
        <span class="badge">PostgreSQL</span>
        <span class="badge">CodeIgniter</span>
        <span class="badge">APIs REST</span>
        <span class="badge">Git & GitHub</span>
      </div>

      <div class="buttons">
        <a href="#" class="btn primary">Ver Portafolio</a>
        <a href="#" class="btn secondary">LinkedIn</a>
        <a href="#" class="btn secondary">GitHub</a>
      </div>
    </div>
  </div>

  <script>
    particlesJS('particles-js', {
      particles: {
        number: { value: 70 },
        color: { value: '#38bdf8' },
        shape: { type: 'circle' },
        opacity: { value: 0.4 },
        size: { value: 3 },
        line_linked: {
          enable: true,
          distance: 150,
          color: '#38bdf8',
          opacity: 0.2,
          width: 1
        },
        move: { enable: true, speed: 2 }
      }
    });
  </script>
</body>
</html>
